...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const isItemActive = (to: string, active?: boolean) => {
    if (currentPath) {
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex lg:flex-shrink-0 lg:fixed lg:inset-y-0">
        ...
        >
          ...
          <div
            ...
            } h-16 px-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700`}
          >
          ...
          <nav className="flex-1 px-2 py-4 space-y-1 overflow-hidden">
            {config.navigation.map((item) => {
              const itemIsActive = isItemActive(item.to, item.active);
              return (
      ...
      {/* Mobile Sidebar */}
      <div
        className={`lg:hidden fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 transform transition-transform duration-300 ease-in-out ${
