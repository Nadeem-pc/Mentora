...
const UserProfilePage: React.FC = () => {
  ...
  const menuItems = [
    ...
    // { icon: Shield, label: 'Logout', id: 'privacy' },
  ];
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center space-x-10">
            ...
            <div className="space-y-2">
              <h1 className="text-4xl font-bold text-white drop-shadow-md">{userProfile?.firstName}</h1>
                    ...
                    return (
                      <div key={index} className="group">
                        ...
                        {isEditing ? (
                          <div className="relative">
                            {field.type === 'select' ? (
                              <select className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-400 transition-all duration-200 bg-white shadow-sm">
                                <option value="">{field.placeholder}</option>
                                {field.options?.map((option) => (
                                  <option key={option} value={option} selected={option === field.value}>
                                    {option}
                                  </option>
                                ))}
                              </select>
